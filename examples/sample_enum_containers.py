...
for i in range(conts.Count):
    ...
    for j in range(cont.Keys.Count):
        ...
        if (hasattr(key, 'PublicKey')):
            logging.info("    PublicKey Algorithm: " + key.PublicKey.Algorithm.Value)
            logging.info("    PublicKey KeyLength: " + str(key.PublicKey.Length))
        ...
        if (key.HasCertificate):
            ...
            logging.info("      Valid From:    " + oCertificate.ValidFromDate + " UTC");
            logging.info("      Valid To:      " + oCertificate.ValidToDate + " UTC");
            logging.info("      Thumbprint:    " + oCertificate.Thumbprint);
            logging.info("      Key Algorithm: " + oPublicKey.Algorithm.FriendlyName);
